...
from .resources import SCHEMA_FILES as RESOURCE_SCHEMA_FILES
from .resources import read_resource

SCHEMA_FILES = {
    kind: filename
    for kind, filename in RESOURCE_SCHEMA_FILES.items()
    if kind != "common_envelope"
}
ID_FIELDS = {
...
class ContractValidator:
    ...
    def __init__(self, *, _schema_dir: Path | None = None) -> None:
        self._schema_dir = _schema_dir
        self.policy = self._read_json_resource("version-policy.json")
        self.schemas = {
            kind: self._read_json_resource(filename)
            for kind, filename in SCHEMA_FILES.items()
        }
        self.envelope = self._read_json_resource("common-envelope.schema.json")
    ...
    def _read_json_resource(self, name: str) -> Any:
        if self._schema_dir is not None:
            return self._read_json(self._schema_dir / name)
...
def main(argv: list[str] | None = None, *, schema_dir: Path | None = None) -> int:
    ...
    validator = ContractValidator(_schema_dir=schema_dir)
