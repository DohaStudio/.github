# DohaStudio Organization 문서 인덱스

> 문서 상태: [제안]
> 마지막 검토일: 2026-08-11

- [Ecosystem Overview](ecosystem-overview.md)
- [Repository Boundary](repository-boundary.md)
- [Provider Contract](provider-contract.md)
- [Common Policies](common-policies.md)
- [Common Folder Layout](common-folder-layout.md)
- [Common Branch Strategy](common-branch-strategy.md)
- [Common Artifact Policy](common-artifact-policy.md)
- [Common License Policy](common-license-policy.md)
- [Common ADR Policy](common-adr-policy.md)
- [Organization Roadmap](common-roadmap.md)
- [공통 명세 `0.1.0` 기준선](specifications/README.md)
- [AI Music Common Contracts](ai-music-common-contracts.md)
- [Common Contract Architecture](architecture/common-ai-contracts.md)
- [Common Contract Roadmap](roadmap.md)
- [Common Contract ADR Index](decisions/README.md)
- [Common Contract 정합성 검토](validation/common-ai-contracts-validation.md)

Contribution·보안·지원은 [CONTRIBUTING](../CONTRIBUTING.md), [SECURITY](../SECURITY.md), [SUPPORT](../SUPPORT.md)를 확인합니다.

이번 Common Contract 변경은 문서 계약만 정의하며 Schema file, code, API, Runtime, Training, Dataset, Worker, Job과 DB를 구현하지 않습니다.
